...
When used as an MPC primitive~\cite{evans2018_pragmatic_mpc}, the threshold
is set to $t < N/2$ (honest majority).  Addition of shared values is free
(each party adds its shares locally), while multiplication requires one
round of communication per gate, giving $O(N)$ messages per multiplication.
This makes Shamir efficient at large party counts.  However, if the
honest-majority assumption is violated ($t \geq N/2$ parties are corrupt),
the corrupt parties can reconstruct every shared secret, and all privacy
guarantees are lost.
